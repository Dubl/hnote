# Generic melody layer: merges the Glass-style fabric over any drum render.
# Extracted from track2_layer.py so new songs don't need per-track copies.
#
# Usage: python melody_layer.py <drums.mid> <out.mid> <nbars>
#
# The fabric: chord pulses whose top line is drawn by an additive cell over a
# 4-tone collection; state timeline built from the melody-good-tagged bars of
# the original glass track (deterministic replay, seed 0x91a55), held 2 bars,
# flipping back ~45% of the time, progressing. Drone / descant / doubled bass
# as shipped in tracks 2-6.
...
def build_states():
    ...
    return [(coll_by_bar[b], pattern_by_bar[b]) for b in GOOD_MELODY_BARS]

def melody_events(nbars):
    states = build_states()
...
def merge_write(drums_path, out_path, nbars):
    events = melody_events(nbars)
    ons, offs = parse(drums_path)
    msgs = []
    ...
    print(f"wrote {out_path}: {len(ons)} drum notes + {len(events)} melody notes, {nbars} bars")

if __name__ == "__main__":
    merge_write(sys.argv[1], sys.argv[2], int(sys.argv[3]))
